...
public class RandomTest {
		...
		while (power) {
			...
			case 1:
				...
				while (repeat) {
					...
					int user = sc.nextInt();
					// 사용자가 입력한 값이 1~100인지 체크해서 처리
					if (!(1 <= user && user <= 100)) {
						System.out.println("입력 범위가 잘못 되었습니다.");
						continue;
					}
					if (com < user) {
						num1++;
			...
			case 2:
				if (score == 0) {
					...
					System.out.printf("현재 최고 기록은 %d회 입니다.", score);
				}
				break;
			case 3:
	...
	public void game2() {
		...
		while (power) {
			...
			switch (select) {
			case 1:
				// 턴게임 -> 사용자가 컴퓨터가 1~3사이 숫자를 외쳐서 숫자가 1씩 커지는 게임
				// -> 이때 31을 외치는 쪽이 패배
				System.out.println("<<GAME START>>");
